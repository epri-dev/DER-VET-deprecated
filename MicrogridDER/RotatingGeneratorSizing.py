...
class RotatingGeneratorSizing(RotatingGenerator, DERExtension, ContinuousSizing):
    ...
    def constraints(self, mask):
        ...
        """
        # parent constraints
        parent_constraints = super(RotatingGeneratorSizing, self).constraints(mask)
        # only keep the last constraint from the parent class when sizing
        ...
        if not self.being_sized():
            ...
            if self.max_rated_power:
                constraint_list += [cvx.NonPos(power_out - cvx.multiply(self.max_rated_power * self.n, on))]
